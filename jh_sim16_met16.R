...
springharbor <- read.csv("Mendota_springharbor_new.csv", header=TRUE) #For Paul only
springharbor$time <- as.POSIXct(strptime(springharbor$time, "%Y-%m-%d %H:%M:%S", tz="EST"))
write.csv(springharbor, "Mendota_springharbor_new.csv", row.names = FALSE, quote=FALSE)
