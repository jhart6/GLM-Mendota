...
abline(0,0,col='slategrey',lty =2,lwd=2)
legend('topright',c(expression(Modeled~CO[2]~Flux),expression(Observed~CO[2]~Flux~(via~CO[2])),expression(Observed~CO[2]~Flux~(via~DO))),lty = c(1,1,1),lwd=c(2,2,2),col=c('black','gold1','darkorange2'))
